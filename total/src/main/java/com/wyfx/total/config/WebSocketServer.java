...
/**
 * 推送消息到企业后台
 * create by wsm on 2019-12-25
 */
@ServerEndpoint("/")//路径必须以/开始 如/{sid}
@Component
@Deprecated
public class WebSocketServer {
    //concurrent包是线程安全Set，用来存放每个客户端对应的MyWebSocket对象。
    ...
    @OnClose
    public void onClose() {
        webSocketSet.remove(this);  //从set中删除链接
    ...
    public void onMessage(String message, Session session) {
        ...
        for (WebSocketServer item : webSocketSet) {
            try {
                //回复消息给客户端
                item.sendMessage("服务端回复相同的消息给客服端：" + message);
            } catch (IOException e) {
                e.printStackTrace();
            }
        }
