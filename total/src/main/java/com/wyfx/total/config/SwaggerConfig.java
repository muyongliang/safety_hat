...
public class SwaggerConfig {
    ...
    private static final String DEFAULT_PATH = "/docs";

    @Bean
    public Docket createRestApi() {
        return new Docket(DocumentationType.SWAGGER_2)
                //不使用默认的响应码401 403 404等
                .useDefaultResponseMessages(false)
                .apiInfo(apiInfo())
                .select()
                .apis(RequestHandlerSelectors.basePackage("com.wyfx.total.controller"))
                .paths(PathSelectors.any())
                .build();
    }

    private ApiInfo apiInfo() {
        return new ApiInfoBuilder().title("智能安全生产管控平台系统企业平台api文档").version("1.0").build();
    }
